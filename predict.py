    ...
    if mode == "predict":
        ...
        for i in range(len(mylist)):
            ...
            for j in range(int(clip.duration_seconds/(overlap/1000))):
                ...
                st = time.time()
                r_image,a,out_class,score = yolo.detect_image(img)
                print('Elapsed time = {}'.format(time.time() - st))

                for j in range(len(out_class)):
                    if out_class[j]=='gunshot' or 'shotgun':
                        indicater=1
                        num_gun=num_gun+1
                        data['filename'].append(mylist[i])
                        data['offset (s)'].append(str(current_s/1000))
                        data['label'].append(out_class[j])
                        data['confidence'].append(str(100*score[j]))
                    elif out_class[j]=='shotgun':
                        s
                    elif out_class[j]=='hn':
                        indicater=0
                        num_hn=num_hn+1
                ...
                if indicater==1:
                    r_image.save('VOCdevkit/VOC2007/detected/img'+mylist[i]+str(current_s)+'.png')
